...
- **US-15 (EPIC)**: Savoir sur quoi travailler — identifier mes points faibles par **thèmes**, pas
  ...
  > - **US-15b** — La pression du temps. **Livrée et mergée** (PR #109, 2026-09-08), sur sa
  >   propre branche d'intégration comme 15a — voir `## Done` pour l'état complet, les sept
  >   tranches, la réserve sur la couverture agentique de la tranche 07 et le geste
  >   `repair:clocks` qui reste dû au demandeur.
...
- **US-32**: Rendre exploitables les axes **phase** et **matériel** — pour que « je m'effondre en
  ...
  >
  > **Grillée le 2026-09-24** — onze décisions, frontière vide. Branche d'intégration
  > `integration/US-32-phase-and-material`. Spec : `.scratch/phase-and-material/SPEC.md`.
  > Sortie de grill : **ADR-0035** (le découpage en phases est celui de lichess), **ADR-0036** (un
  > déséquilibre est une signature, pas un nombre), et `CONTEXT.md` — `Phase` et `Lichess division`
  > amendées, terme **`Material signature`** ajouté.
  >
  > **Ce que le grill a trouvé et qui dépasse la story** : notre découpage en phases s'accordait à
  > celui de lichess dans **2,2 %** des cas, et notre frontière de début venait du **cap du 15ᵉ
  > coup dans 74,1 %** des parties — le critère documenté (« développement achevé ») était la règle
  > **minoritaire**. Mesuré en portant la mesure D14 d'US-15a-bis de **10 à 415 parties**, sur des
  > données déjà en base et sans une seconde de moteur. Décision du demandeur : adopter leur règle
  > **entière**, `mixedness` comprise — transcrite et vérifiée à **415/415**.
  >
  > **Et l'oracle d'ADR-0031 ne discriminait déjà plus sur la finale** : notre seuil à 6 majeures et
  > mineures *était* le leur, donc les 290/290 que D14 lisait comme une corroboration étaient une
  > formule qui se vérifiait elle-même. C'est consigné plutôt que subi.
  >
  > **Passée en Doing le 2026-09-24.** Cinq tranches, en série :
  > `01-la-frontiere-de-phase-devient-celle-de-lichess` (la règle de lichess remplace la nôtre) →
  > `02-les-degats-par-phase-sur-une-partie` (répartition sous le récapitulatif, `flaggedLoss` et
  > `drift` distingués, `Opportunity` à part, phase non atteinte nommée) →
  > `03-la-material-signature-sur-une-partie` (les configurations traversées en finale et ce
  > qu'elles coûtent) → `04-la-table-des-configurations-sur-stats` (taux de résultat par
  > configuration, seuil 3, comptes et taux ensemble) → `05-suite-hp-et-sortie-de-story` (**HITL**).
  >
  > **Deux constats laissés au demandeur** : la **rétroactivité** — 10,9 % des dégâts changent de
...
- **US-26**: Voir la `Confrontation` coup par coup, sur l'échiquier — pour qu'un taux cesse d'être un
  ...
  > **LIVRÉE le 2026-09-11 — [PR #118](https://github.com/Rdulieu/chess-analyst/pull/118) MERGÉE
  > dans `develop`** (`b45f890`, 40 commits). Branche d'intégration
  > `integration/US-26-confrontation-per-move`, **dix PR de sous-travail** (#110 → #117, #119, #120).
  > Gate à la livraison : build OK, **573 tests serveur + 982 client**, `lint` a **tourné et rendu
  > 0**, **FP 01→07 vertes**, **suite HP 3/3 verte** (+ path 0), **aucun finding bloquant**.
  >
  > **Aucun changement de schéma, aucune migration due** (ADR-0015) : le par-coup est **dérivé**.
  > Sortie de grill : `CONTEXT.md` (`Bonne lecture` / `Sous-lecture` / `Sur-lecture`), **ADR-0032**
  > (les chiffres de la `Confrontation` sont le pli de ses lectures par coup) et **ADR-0033** (un
  > échiquier peint un auteur, la cartouche nomme la comparaison).
  >
  > **Ce que la story a coûté en plus de son périmètre, et qui vaut d'être su** : sur sept tranches,
  > **onze findings bloquants, aucun attrapé par les tests** — trouvés par la revue indépendante,
  > par les passes agentiques, ou **par le demandeur sur sa vraie base**. Trois d'entre eux étaient
  > des correctifs partis au vert **sans jamais atteindre l'écran**, par trois mécanismes
  > différents. C'est le matériau d'**US-43**, ouverte pour ça.
  >
  > **Décision de méthode prise en cours de route, et qui déplace un chiffre** : un `Good` que le
  > moteur flague n'est plus « rien à comparer » — c'est une `Sous-lecture`, rouge, **scorée**.
  > Tranchée par le demandeur le 2026-09-11 sur la partie 715, où l'écran disait « rien à opposer à
  > ce verdict » et, deux lignes plus bas, « ce coup vous a coûté des chances ». Sur cette partie
  > l'exactitude passe de **74 % à 70 %**. **US-42 est close par cette décision**, pas reportée.
  >
  > **Réserve sur la couverture agentique** — même forme que celle d'US-15b : la suite HP a été
  > jouée **avant** le dernier round de retours (PR #117, #119, #120). Elle ne connaît donc ni la
  > colonne « Confrontation », ni le `Good` scoré, ni le focus qui suit le plateau, ni la cartouche
  > passée sous l'échiquier. Son vert atteste du code de la veille, et **aucun agent n'a re-joué la
  > forme livrée**.
  >
  > **Sept décisions produit laissées ouvertes** dans le corps de la PR #118 — dont la couche
  > postérieure (que la SPEC dit absente et qu'aucun ticket ne retire, `OUVERT` daté dans la SPEC),
  > `ErrorTallyReadout` qui compte le coup forcé sur l'écran fait pour séparer les deux lectures, et
  > la densité des colonnes titrées (la liste passe de ~390 px à **963 px**, et
  > `denseScreens.test.ts` garde les styles en ligne, **pas la hauteur**).
  >
  > **US ouvertes par cette story** : **US-40** (poser `Sound` sans vingt-deux clics), **US-41**
  > (légende en infobulle), **US-42** (close par décision), **US-43** (la porte qui a dit vert sans
  > rien garantir).
  >
  > <details><summary>Grill, spec et découpage — conservés</summary>
  >
  > **Pas encore grillée.** Ouverte le 2026-09-02 depuis
  > [`docs/feedback/2026-08-25-us16-confrontation.md`](docs/feedback/2026-08-25-us16-confrontation.md).
...
- **US-15b**: La pression du temps — savoir où le temps consommé explique mes fautes, et sur quels
  coups l'horloge basse coïncide avec la dérive.
  > **LIVRÉE le 2026-09-08 — [PR #109](https://github.com/Rdulieu/chess-analyst/pull/109) MERGÉE
  > dans `develop`** (`400ab9a`). Sous-story de l'**EPIC US-15**, sur sa propre branche
  > d'intégration `integration/US-15b-time-pressure`. Gate à la livraison : build OK, **523 tests
  > serveur + 918 client**, `lint` a **tourné et rendu 0**, **FP 01→05 vertes**, **suite HP 3/3
  > verte** (+ path 0), **aucun finding bloquant**.
  >
  > **Réserve sur la couverture agentique** : la suite HP a été jouée contre `9de48ac`, donc
  > **avant la tranche 07**. L'étape 9c de HP-01 portait sur l'ancien emplacement des chiffres ;
  > elle a été réécrite en sondage à la livraison (HP-01, HP-02, HP-03 amendés), mais la forme
  > actuelle n'a **pas** été re-jouée par un agent.
  >
  > **Reste à faire par le demandeur** : passer `npm run repair:clocks -w server -- <db>` sur la
  > vraie base — geste destructeur sur 434 PGN dont 10 parties analysées, vérifié à l'octet sur des
  > copies mais **jamais joué sur l'original**. Tant qu'il ne l'est pas, l'axe `rapid` reste vide.
  >
  > **Trois décisions ouvertes**, laissées au demandeur : le repère « horloge basse » à 10 % du
  > budget initial (choix de l'agent, dans aucune ADR — déclaré au titre d'ADR-0027) ; la catégorie
  > affichée en anglais brut à côté de libellés français ; les coups les plus longs désignés par
  > leur numéro sans le SAN.
  >
  > **Grillée le 2026-09-08** — 24 décisions, frontière vide ; état complet et faits mesurés :
  > `.scratch/time-pressure/GRILL-NOTES.md` (+ le relevé `clock-probe.md`). `CONTEXT.md` : quatre
  > termes ajoutés (**Time control**, **Clock**, **Time spent**, **Lichess division**), `Time
  > control category` amendée ; **ADR-0029** (l'horloge est dérivée du PGN), **ADR-0030** (le
  > rafraîchissement remplace le PGN et refuse un mouvement différent), **ADR-0031** (la division
  > lichess est un oracle, jamais la `Phase`). **Spec** : `.scratch/time-pressure/SPEC.md`.
  > Grillée **entière** — récupérer la donnée de temps *et* l'exploiter (décision du demandeur ; une
  > story amont dédiée à la seule récupération a été proposée puis écartée).
  >
  > **Sept tranches** sous `.scratch/time-pressure/tickets/` :
  > - `01-the-exact-time-control` — la cadence exacte de bout en bout
  > - `02-time-per-move` — le temps par coup dans le relevé ; **livre les 1983 parties chess.com
  >   sans aucun rafraîchissement**
  > - `03-the-game-s-time-reading` — la lecture du temps sur la partie
  > - `04-the-lichess-export-asks-for-clocks` — `clocks`/`division` + la migration
  > - `05-the-cli-brings-back-the-434` — le CLI rapatrie les 434 parties ; c'est **cette** tranche
  >   qui fait exister l'axe `rapid`
  > - `06-graft-onto-hp-01-and-the-pr` — greffe sur HP-01 et la PR
  > - **tranche 07** *(hors découpage initial)* — trois retours du demandeur après avoir vu tourner
  >   01-06 sur la vraie base : le temps **quitte la liste des coups** pour un graphe « Temps par
  >   coup » sous le graphe d'analyse, et gagne la **part de l'horloge** consommée, avec **deux**
  >   classements dans le panneau (par secondes, par part). L'AC de la tranche 02 — les chiffres
  >   « dans le relevé que je lis déjà » — a été **renversée par le demandeur** : c'était le mauvais
  >   endroit, et cela ne se voyait qu'à l'écran.
  >
  ...
  >
  > **Prémisse corrigée le 2026-09-02 : sur lichess, l'horloge n'était pas dans nos données.**
  > `clocks=true` n'était pas envoyé à l'export, donc le PGN arrivait **sans** `[%clk]`. **Chiffres
  > re-mesurés le 2026-09-08** : **1983 parties chess.com sur 1983 portent l'horloge, 0 sur 434 côté
  > lichess** (zéro, pas une), dont **10 parties lichess déjà analysées**. La story n'était donc pas
  > « un parseur, zéro coût moteur » : c'était un paramètre d'export, **plus une reprise d'import**
  > des parties lichess déjà là — et le PGN stocké devait être remplacé, pas complété, ce qui devait
  > à ADR-0015 une assertion « même mouvement » pour que les `Evaluation`s de ces 10 parties
  > survivent.
...
- **US-21**: Remettre l'usine d'accord avec elle-même — pour qu'un agent qui lit la méthode y trouve
  ...
  > **LIVRÉE et MERGÉE dans `develop` le 2026-09-04** — PR https://github.com/Rdulieu/chess-analyst/pull/108, `integration/US-21-US-25-factory-update`.
  > **US-21 et US-25 ont été grillées ensemble et livrées ensemble** : un seul
...
- **US-22**: Rendre la route de lecture agréable à tenir sur trente coups — pour qu'annoter une
  ...
  > **Livrée le 2026-08-31** — PR [#89](https://github.com/Rdulieu/chess-analyst/pull/89) mergée
  > dans `develop` (`dfc1ebe`). Cinq tranches, la suite HP verte. Ses décisions **D5**, **D7** et
  > **D8** sont désormais acquises : `/profiles` ne déborde plus, la lecture personnelle a ses
  > raccourcis clavier, et la liste des coups porte les cinq glyphes de verdict — c'est le socle
  > sur lequel **US-23** part (voir `## To do`).
  >
