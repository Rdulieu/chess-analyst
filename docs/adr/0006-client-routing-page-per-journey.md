...
- This routing/navigation skeleton is **cross-cutting infrastructure**, not US-5 logic. It is delivered by a **dedicated technical enabler US, separate from US-5**, landing before US-5's feature tickets; US-5 then only adds the `/explorer` page on top. (Mirrors US-2's `code-decomposition` technical slice.)
