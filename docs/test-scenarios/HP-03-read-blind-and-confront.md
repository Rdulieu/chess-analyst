...
and only then sets it against the engine's — reading **five** figures side by side and never a single
score. Then they read **three** of them folded over every reading they have sealed. This is
...
> **Five on a Game, three in the fold, and the two counts do not contradict each other.** This
> scenario said "three figures" everywhere until US-32 slice 09, and it was written when three was
> all there was. A Game's `Confrontation` has carried **five** since US-26/US-30: the Player's own
> three — what was examined, what was seen rightly, where the Player looked — and, beside them and
> never fused into them, « Ce que j'ai regardé chez l'adversaire » and « Ce que j'ai lu juste chez
> l'adversaire » (ADR-0034: judging one's own play and spotting what the opponent offers are two
> abilities, and a single rate would erase their disagreement). The fold on `/confrontation` still
> shows **three** — it folds the Player's own. Nothing is broken here; a run counting exactly three
> on a Game's `Confrontation` would produce a **false red**, which is why both numbers are written
> down.
...
8. Open Game B's `Confrontation` → **five figures side by side and no composite**: the Player's own three — what was examined, what was seen rightly, and where the Player looked — and, in their own block beside them, what was regarded of the opponent and what was read right of it. The provenance reads **« Lue à l'aveugle »**, and the `Search regime` behind the engine's side is stated. **Write these figures down**: step 11 compares against them literally.
9. Read the **confusion matrix** → five declared bands against four measured ones (the fourth being "nothing flagged"), every cell carrying a count, the `Bon` row present and said to be unscored. **Spot-check the reconciliation, do not re-add the grid**: the cells outside the `Bon` row are of the right order against the accuracy denominator printed beside them, and no cell is blank where a zero belongs. Read what the screen says about the direction of the bias — or that there is not enough to say. *(The exhaustive sum is `server/test/confrontation.test.ts`'s job — "adds up to the accuracy denominator over its scorable cells" — and it does it in milliseconds. What only this tier can establish is that the grid is **on the screen**, numbered, and beside the denominator it belongs to.)*
...
12. Open **Game A** from "Mes parties" → it opens on **`Sans aide`**, as every Game now does (US-28), *whatever* was chosen on Game B a moment ago. **Ask for `Détaillé` here**, deliberately, so the engine's record is shown for this Game too — this reading is meant to be an informed one, and since US-28 an informed reading is something the Player does rather than something they inherit. Read Game A — declare at least one severity and a `Key moment` — and seal it → its provenance reads **« Lue informée »**. Open its `Confrontation` → the same five figures again, labelled informed.
13. Open **« Mes lectures »** from the navigation → the summary rests on **2 sealed readings**, says **1 lue à l'aveugle, 1 lue informée**, and shows the Player's own **three** figures, separated — the opponent's pair is a per-Game reading and is not folded here. **Spot-check the fold, do not re-add it**: take **one** numerator and its denominator, check they are the sum of the two Confrontations just read, and confirm the matrix is present and folded (its cells are the two grids' cells, not one of them). Look for an axis — by opening, phase or cadence — and for a single score: there is neither.

    > **Why this step does not re-add the fold by hand** — requester's call, 2026-09-09, the same
    > arbitration HP-01's step 9c received. `server/test/confrontation-fold.test.ts` already proves
    > it, and proves more of it than a run can: *"reconciles with the Games it covers, figure by
    > figure"*, *"sums numerators over sums of denominators — never averages the rates"*, and
    > *"folds the matrix whole"*. Re-adding five matrix rows cell by cell on every suite run buys
    > nothing that test has not already bought, and costs minutes of driving.
    >
    > What only this tier can establish is that **« Mes lectures » exists, is reachable from the
    > navigation, and shows the fold at all** — with its provenance split, without an axis, and
    > without a single composite score. None of that is arithmetic, and no unit test sees it.
...
- Step 5: each mark is persisted as it is made; a Move carrying no verdict stays silent (no sentinel value on screen); and the verdict fieldset, on an opponent's Move, **names whose Move it is** — « Mon verdict — coup adverse » — and makes **no claim about scoring at all**.

  > **Rewritten at US-32 slice 09, and the app is not what changed here.** This check used to read
  > "the app says that a verdict on an opponent's Move will not be scored". US-30 made that false:
  > such a verdict **is** confronted, against the `Opportunity` the engine measured, and it is
  > reported in the `Confrontation`'s own pair of figures — 40 % of the Player's marks stopped being
  > discarded. And the repair was **not** to make the legend say the new truth: the reading route is
  > blind by nature, and a legend mentioning an `Opportunity` would tell the Player a measurement of
  > the opponent's Moves exists, and invite them to read whether *this* ply carries one. What
  > becomes of the verdict is the `Confrontation`'s to say, after the seal.
...
- Step 8: **five** figures — the Player's own three, then the opponent pair in its own block (« Ce qui m'a été offert ») — each with its own question named and each carrying **its count beside its rate**, never a rate alone. The opponent pair is **beside** « Ce que j'ai vu juste » and never inside its grid: nothing is fused, and no sixth figure combines them. **No composite**: count the percentages **inside the figures**, and expect one per figure that has a denominator and none besides. *(Do not count percentages in the surrounding prose: the `Drift` explanation contains the phrase "placerait 100 % hors d'atteinte", and a naive scan of the page reads it as a fourth figure.)* A figure whose denominator is zero reads **« Pas de chiffre »** or **« Pas de score »**, never `0 %`. The provenance label is present. The `Search regime` is stated. The `Drift` is reported beside the Key moment figure and said to be outside its division — **when it amounts to at least a point**: under that it is not shown at all, which is deliberate and not a miss.
- Step 9: the matrix has five declared rows and four measured columns; every cell carries a number, zeros included; the diagonal is marked **without relying on colour alone**; the `Bon` row is present and said to be unscored; the cells outside it are **of the right order** against the accuracy denominator shown beside the matrix — a spot check, the exact sum being `server/test/confrontation.test.ts`'s job and not this run's. The bias sentence is either derivable from the cells on screen, or replaced by a statement that there is not enough to conclude — never a confident claim the cells do not support.
...
- Step 13: the summary says how many readings it rests on and how many of each provenance; **one** of its figures is spot-checked as the sum of the two per-Game Confrontations, and the matrix is present and visibly folded (its cells are the two grids', not one of them). The exhaustive reconciliation — every numerator, every denominator, every cell — is `server/test/confrontation-fold.test.ts`'s job, and it does it in milliseconds. There is **no axis** — no control, and no mention of opening, phase or cadence — and no single score anywhere. *(Displayed point values are rounded, and the screen says so: a total may differ by a point from the sum of the rounded parts. That is the rounding, not a reconciliation failure.)*
