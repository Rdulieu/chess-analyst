...
- [ ] Le nombre est une **colonne**, jamais la **clef** : la signature reste ce qui identifie une
      ligne, et `RR vs Q` ne se confond avec aucune autre configuration à +1.
- [ ] Le nombre ne **trie jamais** la table. L'ordre reste la **fréquence** (tranche 06).
- [ ] Le nombre ne **regroupe jamais** les lignes de la table principale : deux configurations au
...
- [ ] Chaque ligne porte le **désaccord matériel** du joueur : ses majeures et mineures moins celles
...
- [ ] Le barème est **écrit à l'écran**, une fois, pas seulement dans le code : un `+1` ne se lit
      pas sans savoir qu'une dame vaut 9 et deux tours 10.
- [ ] `0` s'affiche **`0`**, pas `+0` ni `—` : l'égalité matérielle est une valeur, pas une absence.
- [ ] La colonne est **triable par le serveur ? Non** — voir les trois conditions ci-dessus. Elle
...
- [ ] Une ligne par **bande de désaccord matériel** : `≤ −9`, `−8..−6`, `−5..−3`, `−2..+2`,
      `+3..+5`, `+6..+8`, `≥ +9`. Les bandes sont un choix à assumer à l'écran, pas à cacher.
- [ ] Colonnes : la bande, le nombre de **couples (partie, configuration)**, les résultats V/N/D, le
      win rate. Comptes et taux ensemble, comme partout.
- [ ] **Le dénominateur n'est PAS un nombre de parties, et ça se dit.** Une partie traverse
...
- [ ] **Mitigation écrite, et c'est le cœur du ticket** : à matériel égal (`−2..+2`), le win rate
...
- [ ] **Aucun score composite.** Pas de « performance ajustée au matériel », pas de résidu par
...
- [ ] **Aucun changement d'ordre** de la table des configurations.
- [ ] **Aucun temps moteur, aucune colonne en base, aucune migration** (ADR-0015) : le désaccord se
      dérive de la signature, qui est déjà là.

## Le reste de la porte

- [ ] ADR-0013 : rien porté par la couleur seule — un signe `+`/`−` est du texte, pas une teinte.
- [ ] Les deux tables tiennent dans **380 px**, `[data-scroll="x"]` autour de chaque table, la prose
      **dehors**. US-32 a déjà payé ce défaut une fois (925 px dans une fenêtre de 365).
- [ ] **Coût nul sur `/stats`** : le désaccord est une fonction pure de la chaîne de signature, pas
...
- [ ] Le barème vit **à un seul endroit** et est testé pour lui-même (`Q vs RR` = −1, `— vs —` = 0,
