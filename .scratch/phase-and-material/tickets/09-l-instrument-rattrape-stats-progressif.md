...
**Status:** done
**Delivered:** 2026-09-25 · merge `be6ba16` sur `integration/US-32-phase-and-material` (PR #131,
merge humain en attente) · gate : build vert, **708 tests serveur / 51 fichiers**, **1 123 tests
client / 71 fichiers**, **117 tests outils / 6 fichiers** (`npm run test:tools`, +12), `npm run lint`
**sorti 0**, **Feature Path verte** sur une copie de la base réelle, aucun finding bloquant ouvert.
La suite **HP n'a pas été rejouée** pour cette tranche : elle l'a été le jour même à `5c3c320`
(3/3 + path 0) et rien ici ne touche l'app au-delà d'un message d'erreur — c'est l'instrument et
les textes de scénario qui changent. La revue indépendante a rendu **un bloquant** — `theme-pass.md`
déclarait le trou du niveau de revue refermé alors qu'aucun scénario ne passait `atReviewMode` —
payé avant le merge, avec ses deux non-bloquants de la même famille.
...
- [x] Attendre **le sujet**, pas le silence réseau : `/stats` est posé quand il ne reste plus de
...
- [x] `pendingRequests` garde son garde-fou, mais **une attente qui expire doit se dire** : un
      ...
      haut.
- [x] Tests dans `npm run test:tools` : une page qui met 12 s à se poser **n'est pas** déclarée
      posée à 5 s.

## B — `theme-pass.md` ne connaît pas l'écran en trois vagues

- [x] La passe doit **dire dans quel état elle audite `/stats`** — et attendre l'état posé sur le
      sujet (`section[aria-busy]` disparu), pas sur le silence réseau.
- [x] Et **le trou déjà connu, payé à la main trois passes de suite** : la passe audite `/analyse`
...
- [x] **HP-03 dit « trois figures »** (Goal, étapes 8, 12, 13). L'écran en rend **cinq** depuis
...
- [x] **HP-03 étape 5** demande que l'app dise qu'un verdict sur un coup adverse ne sera pas noté.
...
- [x] **HP-01, HP-02, path 0** : le bouton dit « **Sélectionner et voir mes parties** » depuis US-23,
      les contrôles disent « Sélectionner ».
- [x] **path 0** : les libellés de cadence à l'écran sont `Bullet, Blitz, Rapid, Classical,
...
- [x] **path 0**, §Internals : la copie de snapshot n'est plus `wal_checkpoint(TRUNCATE)` → `.backup`
      depuis le 2026-08-31 ; « le checkpoint a-t-il été refusé » n'est plus une question qui a un sens.
- [x] **HP-02 étape 15** décrit `/stats` par ses seuls groupes de lignes. L'écran porte aussi les
...
- [x] Le bloc dégâts en échec affiche « Erreur : impossible de charger vos dégâts par phase
...
- [x] Build + **`npm test` ET `npm run test:tools`** + `lint` sorti 0 + FP verte + aucun finding
      bloquant.
- [x] La FP de cette tranche doit **prouver le trou refermé** : un écran dont un bloc met plus de
      5 s n'est pas déclaré posé. Le faire sur `Nonomoho` (relecture ~57 s), le seul profil où la
      marge est confortable.
- [x] Aucun scénario n'est modifié pour le faire passer : on corrige le **texte qui décrit mal
