...
that lets the player navigate level by level through the Moves they've actually played across
**all their imported Games**, starting from any recurring Position — merged across Games by
...
- **Global pre-aggregated counters** keyed by (Position FEN, side, Move): total count, win/draw/loss counts (for standard-scoring win rate), and per-time-control-category sub-counts. Reading the explorer is a lookup, not a scan.
...
- **Schema** (Drizzle/SQLite): a table keyed by (Position FEN, side, Move) storing the counters above — distinct from the `games` table — plus the `move_habits_computed` flag on `games`.
...
- **UI on its own page** (`/explorer`, ADR-0006): a drill-down component with a breadcrumb, rendering candidates as both a list and board arrows (thickness/color) via `react-chessboard`'s arrow support; both trigger the same "descend" action. US-5 adds the `/explorer` route and its nav entry (the navigation skeleton itself is delivered by the separate enabler, which US-5 depends on).
